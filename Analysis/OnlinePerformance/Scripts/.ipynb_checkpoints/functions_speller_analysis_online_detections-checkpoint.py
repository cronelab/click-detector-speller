...
def computing_fp(movement, movement_onsetsoffsets, t_after_movement_limit, t_click_onsets):
    ...
    These are not necessarily the number of false positives. This is because some of these "clicks" may be due to
    the speller highlighter cycling back to the first element in a particular row. Therefore, at the time of each
    of these independently occurring "clicks", it is necessary to go back to the video and check whether or not
    they were actual clicks or simply caused by the automated highlighting of a row's first element.
    ...
    for this_click_onset in t_click_onsets:
        ...
        # If there were no accompanying movements associated with the command change, then that is a potential false positive.
...
def saving_click_latencies(block_id, date, dir_save_latencies, folder_bci2k_or_ui, movement, patient_id, t_click_latencies, task):
    """
    DESCRIPTION:
